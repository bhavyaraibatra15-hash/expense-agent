...
async def test_agent_stream_query(agent_app: AgentEngineApp) -> None:
    ...
    message = '{"amount": 50.0, "submitter": "alice@company.com", "category": "software", "description": "IDE License", "date": "2026-06-06"}'
    events = []
    ...
    for event in events:
        ...
        if (
            ...
            or validated_event.output
        ):
            has_content = True
            break

    assert has_content, "Expected at least one event with content or output"
